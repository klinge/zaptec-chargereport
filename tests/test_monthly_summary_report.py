...
from src.models.zaptec_models import InstallationReport, TotalUserChargerReport, UserDetails
...
class TestMonthlySummaryReport:
    ...
    def test_get_data_for_report(self, mock_date_range, mock_api_class, sample_installation_report, mock_logger):
        """Test data retrieval from API"""
        mock_date_range.return_value = (
            "2024-01-01T00:00:00.001", "2024-01-31T23:59:59.999", "January"
        )
        ...
        with patch('src.reports.monthly_summary_report.setup_logger', return_value=mock_logger):
            ...
            mock_api.get_installation_report.assert_called_once_with(
                from_date="2024-01-01T00:00:00.001",
                to_date="2024-01-31T23:59:59.999"
            )
    ...
    @patch('src.reports.monthly_summary_report.ZaptecApi')
    @patch('src.reports.monthly_summary_report.get_previous_month_range')
    def test_generate_report_end_to_end(self, mock_date_range, mock_api_class, sample_installation_report, mock_logger):
        ...
        with patch('src.reports.monthly_summary_report.setup_logger', return_value=mock_logger):
            with patch('src.reports.monthly_summary_report.EmailService', return_value=mock_email_service):
                report = MonthlySummaryReport()
    ...
    @patch('src.reports.monthly_summary_report.ZaptecApi')
    @patch('src.reports.monthly_summary_report.get_previous_month_range')
    def test_generate_report_error_handling(self, mock_date_range, mock_api_class, mock_logger):
        ...
        with patch('src.reports.monthly_summary_report.setup_logger', return_value=mock_logger):
            with patch('src.reports.monthly_summary_report.EmailService', return_value=mock_email_service):
                with patch('src.reports.monthly_summary_report.handle_error') as mock_handle_error:
                    report = MonthlySummaryReport()
                    report.generate_report()
