...
from src.models.zaptec_models import ChargingSession, ChargingSessionResponse, ChargerFirmware
...
class TestInvoicingReport:
    ...
    def sample_charging_sessions(self):
        ...
        firmware = ChargerFirmware(Major=1, Minor=0, Build=0, Revision=0, MajorRevision=1, MinorRevision=0)
    ...
    def test_process_charging_data(self, mock_makedirs, sample_charging_sessions, mock_logger):
        """Test processing of charging session data into DataFrame"""
        with patch('src.reports.invoicing_report.setup_logger', return_value=mock_logger):
            report = InvoicingReport()
    ...
    def test_generate_report_success(self, mock_date_range, mock_api_class, sample_charging_sessions, mock_logger):
        """Test successful report generation end-to-end"""
        # Mock date range
        mock_date_range.return_value = (
            "2024-01-01T00:00:00.001Z", "2024-01-31T23:59:59.999Z", "January"
        )
        ...
        with patch('src.reports.invoicing_report.setup_logger', return_value=mock_logger):
            with patch('src.reports.invoicing_report.EmailService', return_value=mock_email):
    ...
    @patch('src.reports.invoicing_report.ZaptecApi')
    @patch('src.reports.invoicing_report.get_previous_month_range')
    def test_generate_report_api_error(self, mock_date_range, mock_api_class, mock_logger):
        ...
        with patch('src.reports.invoicing_report.setup_logger', return_value=mock_logger):
            with patch('src.reports.invoicing_report.EmailService', return_value=mock_email):
                with patch('src.reports.invoicing_report.handle_error') as mock_handle_error:
                    report = InvoicingReport()
                    report.generate_report()
    ...
    def test_export_to_csv_permission_error(self, mock_makedirs, mock_logger):
        ...
        test_data = {'Objekt-ID': ['G5001'], 'Slutvärde': [25.5], 'Förbrukning': [25.5], 'Kostnad': [70.0], 'Tariff': [2.75]}
        df = pd.DataFrame(test_data)
    ...
    def test_generate_report_filename(self, mock_logger):
        ...
        with patch('src.reports.invoicing_report.setup_logger', return_value=mock_logger):
            with patch('src.reports.invoicing_report.datetime') as mock_datetime:
                ...
                assert filename == 'test_report_20240115.csv'  # REPORT_FILE from .env.test