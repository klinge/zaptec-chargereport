...
class TestAPIFieldMapping:
    ...
    def test_invoicing_report_field_access(self):
        ...
        if len(sessions.Data) > 0:
            ...
            # These are the EXACT field accesses from invoicing_report.py
            # process_charging_data()
            try:
                ...
                assert (
                    isinstance(user_email, str) and "@" in user_email
                ), "UserEmail should be valid email"
                assert (
                    isinstance(user_name, str) and len(user_name) > 0
                ), "UserFullName should be non-empty string"
                assert (
                    isinstance(energy, (int, float)) and energy >= 0
                ), "Energy should be non-negative number"
                assert device_name.startswith(
                    "Plats "
                ), f"DeviceName format changed: {device_name}"

            except AttributeError as e:
                pytest.fail(
                    f"BREAKING CHANGE: Field access failed - {e}. This will break InvoicingReport!"
                )
    ...
    def test_monthly_summary_field_access(self):
        ...
        report = self.api.get_installation_report(from_date, to_date)

        if len(report.totalUserChargerReportModel) > 0:
            user_report = report.totalUserChargerReportModel[0]

            # These are the EXACT field accesses from monthly_summary_report.py
            # generate_summary_report()
            try:
                group_name = (
                    user_report.GroupAsString
                )  # Line 49: report.GroupAsString
                user_details = (
                    user_report.UserDetails
                )  # Line 50: report.UserDetails
                energy = (
                    user_report.TotalChargeSessionEnergy
                )  # Line 51: report.TotalChargeSessionEnergy
                duration = (
                    user_report.TotalChargeSessionDuration
                )  # Line 52: report.TotalChargeSessionDuration
                sessions = (
                    user_report.TotalChargeSessionCount
                )  # Line 55: report.TotalChargeSessionCount
                ...
                if user_details:
                    ...
                    assert isinstance(
                        email, str
                    ), "UserDetails.Email should be string"
                    assert isinstance(
                        full_name, str
                    ), "UserDetails.FullName should be string"

                # Verify data types match business logic expectations
                assert isinstance(group_name, str), "GroupAsString should be string"
                assert isinstance(
                    energy, (int, float)
                ), "TotalChargeSessionEnergy should be numeric"
                assert isinstance(
                    duration, (int, float)
                ), "TotalChargeSessionDuration should be numeric"
                assert isinstance(
                    sessions, int
                ), "TotalChargeSessionCount should be integer"

            except AttributeError as e:
                pytest.fail(
                    f"BREAKING CHANGE: Field access failed - {e}. This will break MonthlySummaryReport!"
                )
    ...
    def test_device_name_format_consistency(self):
        ...
        for device_name in unique_names:
            ...
            assert device_name.startswith(
                "Plats "
            ), f"DeviceName format changed: '{device_name}' (expected 'Plats XX')"

            # Verify we can extract number as expected
            try:
                number_part = device_name.split()[
                    1
                ]  # Line 195: device_name.split()[1]
                int(number_part)  # Should be convertible to int
            except (IndexError, ValueError):
                pytest.fail(
                    f"DeviceName format incompatible with business logic: '{device_name}'"
                )
