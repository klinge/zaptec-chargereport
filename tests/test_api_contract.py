...
class TestZaptecAPIContract:
        ...
        with ZaptecApi() as api:
            ...
            if len(response.Data) > 0:
                ...
                for field, usage in critical_fields.items():
                    assert hasattr(
                        session, field
                    ), f"BREAKING CHANGE: Session missing '{field}' field ({usage})"
                    ...
                    if field in ["StartDateTime", "EndDateTime"]:
                        ...
                        ), f"Field '{field}' should be datetime, got {type(field_value)}"
                    elif field == "Energy":
                        assert isinstance(
                            field_value, (int, float)
                        ), f"Field '{field}' should be numeric, got {type(field_value)}"
                    elif field in ["UserEmail", "UserFullName", "DeviceName", "UserId"]:
                        assert isinstance(
                            field_value, str
                        ), f"Field '{field}' should be string, got {type(field_value)}"

            # VERIFY: Pagination is handled correctly
            if response.Pages > 1:
                # This is fine now - we handle pagination
                print(
                    f"✓ Pagination working: fetched {len(response.Data)} items across {response.Pages} pages"
                )
    ...
    def test_installation_report_response_structure(self):
        ...
        with ZaptecApi() as api:
            ...
            # Verify response structure
            assert hasattr(
                response, "totalUserChargerReportModel"
            ), "Missing totalUserChargerReportModel"
            assert isinstance(
                response.totalUserChargerReportModel, list
            ), "totalUserChargerReportModel should be list"
            ...
            if len(response.totalUserChargerReportModel) > 0:
                ...
                for field, usage in critical_report_fields.items():
                    assert hasattr(
                        user_report, field
                    ), f"BREAKING CHANGE: Report missing '{field}' field ({usage})"
                    ...
                    if field in ["TotalChargeSessionCount"]:
                        assert isinstance(
                            field_value, int
                        ), f"Field '{field}' should be int, got {type(field_value)}"
                    elif field in [
                ...
                if user_report.UserDetails:
                    ...
                    for field, usage in critical_user_fields.items():
                        assert hasattr(
                            user_details, field
                        ), f"BREAKING CHANGE: UserDetails missing '{field}' field ({usage})"
                        field_value = getattr(user_details, field)
                        assert isinstance(
                            field_value, str
                        ), f"UserDetails.{field} should be string, got {type(field_value)}"
    ...
    def test_data_completeness_check(self):
        ...
        with ZaptecApi() as api:
            ...
            summary_response = api.get_installation_report(
